 ...
 * 跨服务查询用户信息的统一入口。
 *
 * <p>动态表里冗余了 {@code author_name}，但用户改昵称后快照就会过期。
 * 这里通过 Feign 实时向 auth-service 取权威数据，冗余字段退化为兜底。
 *
 * <p>降级策略：任何异常都返回空 Map，由调用方保留本地冗余字段继续渲染。
 */
@Slf4j
@Component
    ...
    public Map<Long, UserBrief> byIds(Collection<Long> userIds) {
        ...
        try {
            ...
            if (response == null || response.getCode() != 0 || response.getData() == null) {
                log.warn("[社区] 用户信息查询返回异常，降级使用本地冗余字段 code={}",
            ...
            log.warn("[社区] 调用 auth-service 查询用户失败，降级使用本地冗余字段: {}", e.getMessage());
            return Map.of();
        }
    }
}
